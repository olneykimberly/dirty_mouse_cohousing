...
gene_names <- dge.filtered.norm$genes$gene_name

# which ones are duplicated
dup_genes <- gene_names[duplicated(gene_names) | duplicated(gene_names, fromLast = TRUE)]

# unique list of duplicated gene names
unique_dup_genes <- unique(dup_genes)
unique_dup_genes # [1] "Pakap" "Aldoa"

# Make gene names unique (add .1, .2, etc.)
dge.filtered.norm$genes$gene_name <- make.unique(dge.filtered.norm$genes$gene_name)
table(duplicated(dge.filtered.norm$genes$gene_name))
```
...
info <- extract_gene_cpm("Hbb-bs", cpm_gene_names, info)
...
plot_data <- info %>%
  dplyr::select(sample, group, Uty, Omp, 'Hbb-bs', Ttr) %>%
  tidyr::gather(key = "Gene", value = "Expression", Uty, Omp, 'Hbb-bs', Ttr) %>%
  dplyr::mutate(
    # Order the groups for plotting
    group = factor(group, levels = c("Clean", "Bedding", "CH")),
    # Order the genes for plotting
    Gene = factor(Gene, levels = c("Uty", "Omp", "Hbb-bs", "Ttr"))
  )

# Create a list of plots, one for each gene
gene_plots <- list()
genes_to_plot <- c("Uty", "Omp", "Hbb-bs", "Ttr") # Only plotting the newly requested genes
...
max_n <- max(df_plot$n, na.rm = TRUE)
x_lim <- max_n * 1.15
...
outdir <- paste0("results_", projectID, "/both_sexes/DEGs")
dir.create(outdir, recursive = TRUE, showWarnings = FALSE)

pdf(
  file = paste0(outdir, "/DEGs_bidirectional.pdf"),
  width = 6,
  height = 5
)
print(p)
...
# reorder columns in the exact order desired
# note: UpSetR can display sets in reverse visually, so this order may need
# to be reversed if the final plot appears flipped top-to-bottom
data <- data[, c(
...
outdir <- paste0("results_", projectID, "/both_sexes/upset")
pdf_file <- paste0(outdir, "/UpSet_DEGs_q0.05.pdf")
txt_file <- paste0(outdir, "/UpSet_list_binary_results.txt")


pdf(pdf_file, width = 6, height = 4.5)
upset_gene <- UpSetR::upset(
  data,
  sets = c(
    "CH vs clean down-regulated",
    "CH vs bedding down-regulated",
  ...
  line.size = 0.8,
  mb.ratio = c(0.6, 0.4),
  text.scale = c(1.2, 1.2, 1, 1, 1.1, 1.1)
)
upset_gene
dev.off()
